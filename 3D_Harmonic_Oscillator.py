...
h, x, y, z, r, s = sp.symbols('h, x, y, z, r, s')


def G_d(x_new, x_old, y_new, y_old, z_new, z_old, delta_tau):
    """
    Importance sampling diffusive displacement probability density distribution for walkers. 
    ...
    The value corresponds to the product of the probability densities for the independent x, y and z displacements.
        ...
        Parameters:
            x[/y/z]_old (float): The original x[/y/z] position of a walker
            x[/y/z]_new (float): The new x[/y/z] position of a walker
            delta_tau (float): The time incrament

        Returns:
            Probability density (float): The probability density of a walker moving from the specified old coordinates
                                         to the specified new coordinates
    """
    return np.exp(-((x_new - x_old - delta_tau * velocity_x(x_old))**2)/(2*delta_tau)) * \
           np.exp(-((y_new - y_old - delta_tau * velocity_y(y_old))**2)/(2*delta_tau)) * \
           np.exp(-((z_new - z_old - delta_tau * velocity_z(z_old))**2)/(2*delta_tau))


def importance_sampling_weight(old_local_energies, new_local_energies, energy, delta_tau):
    """
    Time-dependent renormalization (reweighting) of the diffusion Green’s function
        ...
        Parameters:
            ...
            energy (float): The value of the Diffusion Monte Carlo reference/ offset energy
            delta_tau (float): The time incrament

        Returns:
            Weight value (float): The renormalised weight of the walker associated with the given step
    """
    return np.exp(- delta_tau * (new_local_energies + old_local_energies - 2*energy) / 2)


def walking_and_branching(walkers, E_T_energies, w_E_L, sum_weights, num_steps, \
                          all_x_positions, all_y_positions, all_z_positions, \
    ...
    Performs the walking and branching steps of the Diffusion Monte Carlo process.
        ...
        Returns:
            ...
            w_E_L (float): An array containing the value of the sum of the product of the weight and local energy
                           after each step
            all_x[/y/z]_positions (float): An array containing the x[/y/z] position of the walker after each measured step
    ...
    for step in range(num_steps):
        ...
        y_old = walkers[2, :]
        z_old = walkers[3, :]
        x_proposed = x_old + np.random.normal(scale=np.sqrt(delta_tau), size=N_max) + delta_tau * velocity_x(x_old)
        y_proposed = y_old + np.random.normal(scale=np.sqrt(delta_tau), size=N_max) + delta_tau * velocity_y(y_old)
        z_proposed = z_old + np.random.normal(scale=np.sqrt(delta_tau), size=N_max) + delta_tau * velocity_z(z_old)
        ...
        for i in range(len(walkers[1])):
            ...
            if walkers[0, i] == 1:
                ...
                p = np.minimum(1, (G_d(x_old[i], x_proposed[i], y_old[i], y_proposed[i], z_old[i], z_proposed[i], delta_tau) \
                                   * (trial_psi(x_proposed[i], y_proposed[i], z_proposed[i]))**2) \
                                / (G_d(x_proposed[i], x_old[i], y_proposed[i], y_old[i], z_proposed[i], z_old[i], delta_tau) \
                                   * (trial_psi(x_old[i], y_old[i], z_old[i]))**2))
        ...
        E_T = np.mean(potential(walkers[1, walkers[0,:] == 1], \
                                walkers[2, walkers[0,:] == 1], \
                                walkers[3, walkers[0,:] == 1])) + alpha * (1 - N_alive / N_0)
...
def diffusion_monte_carlo(N_0, N_max, num_steps_thermalize, num_steps_measure, max_duplicates, delta_tau):
    ...
    walkers = np.zeros((4, N_max))

    walkers[0, :N_0] = 1            # Set the first N_0 walkers to be alive
    walkers[1:, :] =  np.random.uniform(-1.5, 1.5, [3,N_max])   # Starting the walkers somewhat spread out from origin
...
if __name__ == "__main__":
    ...
    num_steps_thermalize = 5000   # Number of steps to thermalise the system
    ...
    n_bins = 2500                 # Number of data bins for wavefunction histogram
    ...
    potential_symbolic = 0.5 * (x**2 + y**2 + z**2)
    trial_psi_symbolic = sp.exp(-(x**2+y**2+z**2)/3)
    r = np.linspace(0, 5, n_bins)
    def trial_psi_r(r):
        return np.exp(-(r**2)/3)
    ...
    velocity_x = sp.lambdify(x, velocity_x_symbolic)
    velocity_y = sp.lambdify(y, velocity_y_symbolic)
    velocity_z = sp.lambdify(z, velocity_z_symbolic)
    ...
    plt.axhline(y=1.5, color='tab:red', linestyle='dashed', label='Exact Energy')
    plt.legend()
    ...
    psi_r = np.exp(-r**2/2)
    Area_r_int = np.trapz(r**2*(psi_r)**2, r)
    Normalised_psi_r = psi_r / np.sqrt(Area_r_int)
    # Plot the calculated and exact wavefunctions
    plt.figure()
    ...
    f_hist_values_r, f_bin_edges_r = np.histogram(measurements_r_positions, bins=n_bins, range=[0, 5], density=True)
    wavefunction_hist_values_r = f_hist_values_r / trial_psi_r(r)
    wavefunction_hist_values_r = (wavefunction_hist_values_r) / np.sqrt(np.sum(r[1:]**(-2)*(wavefunction_hist_values_r[1:])**2 * 5/n_bins))
    ...
    plt.legend()
    plt.title(r'3D HO Ground State $\Psi$(r)')
